...
<head>
  ...
  <meta property="og:image" content="http://www.chartersearch.com/wp-content/uploads/2013/08/chartersearch-social.png" />
  <meta http-equiv="X-UA-Compatible" content="IE=edge,chrome=1" />
  <meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=5" />
  <title>CharterSearch | Collegiate Athletic Charters, Corporate Charters & Political Campaign Charters</title>
  <link rel="pingback" href="<?php bloginfo('pingback_url'); ?>" />
  <?php if (is_singular() && get_option('thread_comments')) wp_enqueue_script('comment-reply'); ?>
  <?php if(ot_get_option('uxbarn_to_setting_upload_favicon')) : ?>
...
<div class="fullcontainer" style="width: 100%; position: fixed; z-index: 55;">
  <div id="header-container" class="content-width <?php echo $header_style_class; ?>" style="position: initial; left: 0; margin: 0px auto; right: 0;">
    ...
    <div id="menu-wrapper">
      <?php
        $args = array('theme_location'=>'header-menu','menu_class'=>'sf-menu','menu_id'=>'root-menu','fallback_cb'=>false);
        wp_nav_menu($args);
      ?>
    ...
    <?php if(function_exists('icl_get_languages') && ot_get_option('uxbarn_to_setting_display_theme_wpml_lang_selector')) : // If WPML plugin is active, display lang selector. ?>
...
<?php if(is_front_page() || uxbarn_is_frontpage_child() && !is_search() && !is_single()) : // Use home slider on front page ?>
  ...
  <?php if($slider_type == 'basic-slider') : ?>
          ...
          <?php $i = 1; while($slides->have_posts()) : $slides->the_post(); ?>
            ...
            <div id="<?php echo $slide_id; ?>" class="home-slider-item">
              <?php
                ...
                $show_caption = uxbarn_get_array_value(uxbarn_get_array_value(get_post_meta($post->ID,'uxbarn_homeslider_caption_display'), 0), 0);
                  // Define caption ID
              ...
              <?php if($show_caption) : ?>
                <div id="<?php echo $slide_caption_id; ?>" class="slider-caption">
                  ...
                  <p class="caption-body">
                    <?php echo $show_caption = uxbarn_get_array_value(get_post_meta($post->ID,'uxbarn_homeslider_caption_body'), 0); ?>
                  </p>
                </div>
              <?php endif; ?>
            </div>
    ...
    </div><!-- END: id="home-slider-container" -->
  <?php else : // else if $slider_type == 'layerslider' ?>
    <?php
      ...
      $no_slider_class = '';
      if($layerslider_id == '') { // For never-saved option (no value in DB)
        $layerslider_id = '-1';
      } else if($layerslider_id == '-1') { // For non-selected slider
        $layerslider_id = '-1';
      ...
      } else { // else if it's selected, display on screen.
